 ...
 *   fail-connect-N:<count>      — fail the first <count> transport connect() attempts
 ...
 *
 * Two further modes — `inspector-unreachable` and `inspector-adopt` — went
 * away with the ngrok agent inspector they drove. Both existed to exercise
 * re-adoption of a tunnel already registered with the local ngrok agent, a
 * mechanism that the per-session-tunnel cutover retired ("no sharing, ever")
 * and that the debugg transport has no equivalent of: it owns its own socket
 * and there is no out-of-process agent holding tunnels to discover.
 */
